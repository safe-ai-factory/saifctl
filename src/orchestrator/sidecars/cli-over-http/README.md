...
The orchestrator injects two files via `putArchive` (sidecar and staging-start.sh must be executable; bind mounts can lose the +x bit). stage.sh is mounted read-only instead — it's invoked via `sh /saifctl/stage.sh` so it need not be executable.

```
/saifctl/sidecar          ← injected via putArchive (Go binary, mode 0755)
/saifctl/staging-start.sh ← injected via putArchive (entrypoint script)
/saifctl/stage.sh         ← mounted from host (profile's startup script, e.g. pnpm run start)
```
...
Once `container.start()` is called, the container runs `/bin/sh /saifctl/staging-start.sh`. Here is exactly what happens, in order:

```
1. sh /saifctl/startup.sh
...
2. /saifctl/sidecar &
   ...
   It binds to 0.0.0.0:$SAIFCTL_SIDECAR_PORT immediately.

3. sh /saifctl/stage.sh
...
These values come from `saifctl/config.ts` (`environments.staging.app`) and are injected by the orchestrator as container environment variables (`SAIFCTL_SIDECAR_PORT`, `SAIFCTL_SIDECAR_PATH`).
