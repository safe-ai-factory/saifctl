...
`;
  ...
  consola.log(`[design-tests:rust-playwright] Written ${configPath}`);
}
...
`;
  ...
  consola.log(`[design-tests:rust-playwright] Written ${cargoTomlPath}`);
}
...
async function rustPlaywrightOnDone(opts: OnDoneOpts): Promise<void> {
  ...
  for (const subdir of ['public', 'hidden'] as const) {
    ...
    if (!(await pathExists(modRsPath)) || force) {
      ...
      consola.log(`[design-tests:rust-playwright] Written ${modRsPath}`);
