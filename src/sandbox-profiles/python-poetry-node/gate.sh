#!/bin/bash
# python-poetry-node sandbox profile — default gate script.
...
# Common options: poetry run pytest, poetry run ruff check ., etc.
set -euo pipefail
cd /workspace
...
echo "[factory-gate] WARNING: The python-poetry-node profile has no default gate."
