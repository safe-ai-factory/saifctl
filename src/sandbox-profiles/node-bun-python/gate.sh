#!/bin/bash
# node-bun-python sandbox profile — default gate script.
# WARNING: This is a placeholder. There is no standard built-in "check" command.
# You MUST supply a project-specific gate script via --gate-script.
# Common options: bun test, bun run lint, etc.
set -euo pipefail
cd /workspace
...
echo "[factory-gate] WARNING: The node-bun-python profile has no default gate."
