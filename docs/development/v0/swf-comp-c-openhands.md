...
_(Note: In the actual Factory loop, when Leash is enabled we run `npx leash --image saifac-coder-node-pnpm-python:latest --volume <sandbox>:/workspace --policy leash-policy.cedar ... openhands ...`. Leash wraps OpenHands in a sandboxed Docker container with Cedar policy enforcement. Use `--dangerous-debug` to run OpenHands directly on the host.)_
