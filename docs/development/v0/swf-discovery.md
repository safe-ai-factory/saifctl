# Software Factory Discovery Step (`saif feat design-discovery`)
...
When the user runs `saif feat design` (or `saif feat design-discovery`), the pipeline checks if any discovery tools (`discoveryMcps` or `discoveryTools`) are configured via CLI arguments or the project `config.json`.
...
5. Writes the agent's output to `saif/features/<name>/discovery.md`.

Once `discovery.md` exists, the subsequent `saif feat design-specs` step automatically detects it and injects it into Shotgun alongside the original `proposal.md`. Shotgun treats this discovered context as part of the project reality, using it to inform the final specs and test plans.
...
The `design-discovery` command is cleanly separated but orchestrated into the larger `saif feat design` flow.
