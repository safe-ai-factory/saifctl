...
Add the new provider to this document (section 4) and to user-facing CLI docs — e.g. `docs/commands/feat-run.md` / `docs/commands/run-start.md` and the `--git-provider` entry in `src/cli/args.ts` (`featRunArgs`).
