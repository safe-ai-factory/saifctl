...
Shows factory sandbox entries in `/tmp/factory-sandbox/` (or a custom path). By default lists only entries for the current project (from `package.json` name). Use `--all` to list entries for all projects.
